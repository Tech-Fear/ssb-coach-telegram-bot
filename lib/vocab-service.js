const { CONTENT } = require("./content");

const DATAMUSE_BASE = "https://api.datamuse.com/words";
const DICTIONARY_BASE = "https://api.dictionaryapi.dev/api/v2/entries/en";
const VOCAB_TOPICS = [
  "leadership",
  "discipline",
  "courage",
  "service",
  "integrity",
  "teamwork",
  "learning",
  "fitness",
  "resilience",
  "judgment",
  "planning",
  "responsibility",
  "communication",
  "decision-making",
  "problem-solving",
  "confidence",
  "punctuality",
  "loyalty",
  "initiative",
  "maturity",
  "adaptability",
  "credibility",
  "commitment",
  "efficiency",
  "humility",
  "accountability",
  "cooperation",
  "perseverance",
  "composure",
  "fairness",
  "honesty",
  "patriotism",
  "valor",
  "sacrifice",
  "duty",
  "honor",
  "motivation",
  "innovation",
  "strategy",
  "vision",
  "excellence",
  "ethics",
  "empathy",
  "diplomacy",
  "negotiation",
  "conflict-resolution",
  "mentorship",
  "excellence",
  "courage",
  "integrity",
  "wisdom",
];
const STOPWORDS = new Set([
  "the",
  "and",
  "for",
  "with",
  "that",
  "from",
  "this",
  "into",
  "about",
  "than",
  "have",
  "been",
  "will",
  "your",
  "their",
  "them",
  "were",
  "which",
  "while",
]);

function titleCase(word) {
  return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
}

async function fetchJson(url) {
  const response = await fetch(url, { signal: AbortSignal.timeout(8000) });
  if (!response.ok) {
    throw new Error(`Request failed: ${response.status}`);
  }
  return response.json();
}

function extractKeywords(definition) {
  return (definition || "")
    .toLowerCase()
    .replace(/[^a-z ]/g, " ")
    .split(/\s+/)
    .filter((word) => word.length > 3 && !STOPWORDS.has(word))
    .slice(0, 4);
}

async function fetchDictionaryEntry(word) {
  try {
    const entries = await fetchJson(
      `${DICTIONARY_BASE}/${encodeURIComponent(word.toLowerCase())}`,
    );
    if (!Array.isArray(entries) || !entries.length) return null;

    const entry = entries[0];
    const allMeanings = [];

    for (const meaning of entry.meanings || []) {
      const definitionBlock = meaning.definitions?.find(
        (item) => item.definition,
      );
      if (!definitionBlock) continue;
      allMeanings.push({
        partOfSpeech: meaning.partOfSpeech || "Word",
        meaning: definitionBlock.definition,
        example: definitionBlock.example || null,
        synonym: definitionBlock.synonyms?.[0] || meaning.synonyms?.[0] || null,
        antonym: definitionBlock.antonyms?.[0] || meaning.antonyms?.[0] || null,
        meaningKeywords: extractKeywords(definitionBlock.definition),
      });
    }

    return allMeanings.length > 0 ? allMeanings : null;
  } catch {
    return null;
  }
}

async function fetchDatamuseWordList(topic) {
  try {
    const rows = await fetchJson(
      `${DATAMUSE_BASE}?ml=${encodeURIComponent(topic)}&max=25`,
    );
    return rows
      .map((row) => row.word)
      .filter(
        (word) =>
          /^[a-zA-Z-]+$/.test(word) && word.length >= 4 && word.length <= 12,
      )
      .map((word) => word.toLowerCase());
  } catch {
    return [];
  }
}

async function fetchDatamuseRelation(word, relation) {
  try {
    const rows = await fetchJson(
      `${DATAMUSE_BASE}?rel_${relation}=${encodeURIComponent(word.toLowerCase())}&max=5`,
    );
    return rows.find((row) => /^[a-zA-Z-]+$/.test(row.word))?.word || null;
  } catch {
    return null;
  }
}

async function fetchHindiTranslation(word) {
  try {
    const response = await fetch(
      `https://api.mymemory.translated.net/get?q=${encodeURIComponent(word)}&langpair=en|hi`,
      { signal: AbortSignal.timeout(5000) },
    );
    if (!response.ok) return null;
    const data = await response.json();
    const translation = data?.responseData?.translatedText;
    return translation && translation.toLowerCase() !== word.toLowerCase()
      ? translation
      : null;
  } catch {
    return null;
  }
}

async function fetchExampleSentence(word) {
  try {
    const response = await fetch(
      `https://www.wordsapi.com/words/${encodeURIComponent(word.toLowerCase())}/examples`,
      { signal: AbortSignal.timeout(5000) },
    );
    if (!response.ok) return null;
    const data = await response.json();
    return data.examples?.[0] || null;
  } catch {
    return null;
  }
}

async function fetchMerriamWebsterExample(word) {
  try {
    const response = await fetch(
      `https://www.dictionaryapi.com/api/v3/references/collegiate/json/${encodeURIComponent(word.toLowerCase())}`,
      { signal: AbortSignal.timeout(5000) },
    );
    if (!response.ok) return null;
    const data = await response.json();
    if (Array.isArray(data) && data[0]?.def) {
      const def = data[0].def[0];
      if (def.sseq) {
        for (const seqItem of def.sseq) {
          const sense = Array.isArray(seqItem) ? seqItem[0] : seqItem;
          if (sense[1]?.vis && sense[1].vis[0]?.t) {
            return sense[1].vis[0].t.replace(/{[^}]+}/g, word);
          }
        }
      }
    }
    return null;
  } catch {
    return null;
  }
}

function generateContextualExample(word, partOfSpeech, meaning, synonym) {
  const cleanWord = titleCase(word);
  const lowerWord = word.toLowerCase();
  const keywords = extractKeywords(meaning);

  // Extract key concepts from meaning
  const isTaughtConcept =
    meaning?.toLowerCase().includes("teach") ||
    meaning?.toLowerCase().includes("instruct") ||
    meaning?.toLowerCase().includes("educate");
  const isPositiveQuality =
    meaning?.toLowerCase().includes("good") ||
    meaning?.toLowerCase().includes("kind") ||
    meaning?.toLowerCase().includes("friend");
  const isAuthoritative =
    meaning?.toLowerCase().includes("author") ||
    meaning?.toLowerCase().includes("permit") ||
    meaning?.toLowerCase().includes("warrant") ||
    meaning?.toLowerCase().includes("sanction");
  const isPunitive =
    meaning?.toLowerCase().includes("penali") ||
    meaning?.toLowerCase().includes("punish") ||
    meaning?.toLowerCase().includes("infring") ||
    meaning?.toLowerCase().includes("rule");

  const kind = (partOfSpeech || "").toLowerCase();

  if (kind.includes("adjective")) {
    if (isTaughtConcept) {
      return `The ${lowerWord} method of teaching ensures students grasp complex concepts thoroughly.`;
    }
    if (isPositiveQuality) {
      return `Her ${lowerWord} personality made her well-liked among colleagues and friends.`;
    }
    return `The team appreciated her ${lowerWord} approach to solving the problem.`;
  }

  if (kind.includes("noun")) {
    if (isAuthoritative) {
      return `A ${lowerWord} is required before proceeding with this decision.`;
    }
    return `Understanding ${cleanWord} is crucial in this context.`;
  }

  if (kind.includes("verb")) {
    if (isPunitive) {
      return `Coaches may ${lowerWord} players who breach team regulations.`;
    }
    if (isTaughtConcept) {
      return `Teachers ${lowerWord} complex ideas through clear examples and discussions.`;
    }
    return `Leaders should ${lowerWord} effective communication within their team.`;
  }

  if (kind.includes("adverb")) {
    return `The task was completed ${lowerWord} and with high quality.`;
  }

  return null;
}

async function enrichVocabItem(item) {
  const dictionaryMeanings = await fetchDictionaryEntry(item.word);

  // Select the primary meaning - prefer user-provided partOfSpeech or first one
  let dictionary = null;
  if (dictionaryMeanings && Array.isArray(dictionaryMeanings)) {
    if (item.partOfSpeech) {
      dictionary =
        dictionaryMeanings.find(
          (m) =>
            m.partOfSpeech.toLowerCase() === item.partOfSpeech.toLowerCase(),
        ) || dictionaryMeanings[0];
    } else {
      dictionary = dictionaryMeanings[0];
    }
  }

  const synonym =
    dictionary?.synonym ||
    item.synonym ||
    (await fetchDatamuseRelation(item.word, "syn")) ||
    "related";
  const antonym =
    dictionary?.antonym ||
    item.antonym ||
    (await fetchDatamuseRelation(item.word, "ant")) ||
    "opposite";
  const partOfSpeech = dictionary?.partOfSpeech || item.partOfSpeech || "Word";
  const meaning =
    dictionary?.meaning ||
    item.meaning ||
    `A useful English word related to ${item.word.toLowerCase()}.`;

  let hindiMeaning = item.hindiMeaning || null;
  if (!hindiMeaning) {
    hindiMeaning = await fetchHindiTranslation(item.word);
  }

  return {
    ...item,
    partOfSpeech,
    meaning,
    hindiMeaning,
    example:
      dictionary?.example ||
      (await fetchExampleSentence(item.word)) ||
      (await fetchMerriamWebsterExample(item.word)) ||
      item.example ||
      generateContextualExample(item.word, partOfSpeech, meaning, synonym),
    synonym,
    antonym,
    meaningKeywords: dictionary?.meaningKeywords?.length
      ? dictionary.meaningKeywords
      : item.meaningKeywords || extractKeywords(meaning || item.word),
    allMeanings: dictionaryMeanings || [],
  };
}

async function enrichVocabItems(items) {
  return Promise.all(items.map((item) => enrichVocabItem(item)));
}

async function getVocabPool() {
  const dynamicWords = new Set();
  const shuffledTopics = [...VOCAB_TOPICS]
    .sort(() => Math.random() - 0.5)
    .slice(0, 5);

  for (const topic of shuffledTopics) {
    const words = await fetchDatamuseWordList(topic);
    for (const word of words) {
      dynamicWords.add(word);
      if (dynamicWords.size >= 120) break;
    }
    if (dynamicWords.size >= 120) break;
  }

  const dynamicPool = [...dynamicWords].map((word) => ({
    key: `vocab-${word}`,
    type: "vocab",
    word: titleCase(word),
  }));

  const fallbackPool = (CONTENT.vocab || []).map((item) => ({ ...item }));
  const seen = new Set(dynamicPool.map((item) => item.key));
  for (const item of fallbackPool) {
    if (!seen.has(item.key)) {
      dynamicPool.push(item);
      seen.add(item.key);
    }
  }

  return dynamicPool;
}

module.exports = {
  getVocabPool,
  enrichVocabItem,
  enrichVocabItems,
};
